...
if opt.model == 'real-data':
    ...
    syn_data = model.generate(5000, class_ratios)
...
if opt.downstream_task == "classification": # important
    ...
    names = ['LR', 'BernoulliNB', 'GaussianNB', 'XGBoost', 'RandomForest', 'Bagging', 'AdaBoost', 'ExtraTree', 'DecisionTree', 'MLP', 'GradientBoost', 'Ave', 'AveLR_NN']
    ...
    for i in range(0, len(learners)):
        ...
        if names[i] in ['LR', 'MLP']:
            AUC_LRNN += round(auc_score, 3)
    AUC.append(round(np.array(AUC).mean(), 3))
    AUC.append(round(AUC_LRNN/2.0, 3))
