...
from sklearn.ensemble import RandomForestClassifier, GradientBoostingClassifier, BaggingRegressor, BaggingClassifier, AdaBoostClassifier, ExtraTreesClassifier
from sklearn.linear_model import LogisticRegression
...
parser.add_argument('--epoch', type=int, default=100, help='number of iteration') # fix epoch and
...
privacy_parser.add_argument('--target-epsilon', type=float, default=1e10, help='Epsilon differential privacy parameter') #####  important
...
parser_pate_gan.add_argument('--num-teachers', type=int, default=5, help="Number of teacher disciminators in the pate-gan model")
...
# opt.model = 'dq-wgan'
# opt.target_epsilon = 1e10
# opt.target_delta = 1e-5
# opt.batch_size = 64
...
# opt.target_variable='y'
# opt.train_data_path='./data/MNIST_CNN_train.csv'
# opt.test_data_path='./data/MNIST_CNN_test.csv'
...
if opt.model == 'pate-gan':
    ...
    model = pate_gan.PATE_GAN(input_dim, z_dim, opt.num_teachers, opt.target_epsilon, opt.target_delta, conditional, opt.epoch)
    model.train(X_train, y_train, Hyperparams(batch_size=opt.batch_size, num_teacher_iters=opt.teacher_iters,
                                              ...
                                              lap_scale=opt.lap_scale, class_ratios=class_ratios, lr=2e-4))
    ...
    model = dp_wgan.DP_WGAN(input_dim, z_dim, opt.target_epsilon, opt.target_delta, conditional, opt.epoch)
    model.train(X_train, y_train, Hyperparams(batch_size=opt.batch_size, micro_batch_size=opt.micro_batch_size,
                                              ...
                                              2e-4, num_epochs=opt.num_epochs), private=opt.enable_privacy)
...
# step 3: Generating synthetic data from the trained model
if opt.model == 'real-data':
    ...
    # syn_data = model.generate(X_test.shape[0], class_ratios)
    syn_data = model.generate(200, class_ratios)
...
# step 4: Creating downstream learners: test the classifier
learners = []

if opt.downstream_task == "classification": # important
    # names = ['LR', 'Random Forest', 'Neural Network', 'GaussianNB', 'GradientBoostingClassifier', 'BaggingClassifier', 'AdaBoostClassifier', 'ExtraTreesClassifier']
    names = ['LR', 'BernoulliNB', 'GaussianNB', 'XGBoost', 'RandomForest', 'Bagging', 'AdaBoost', 'ExtraTree', 'DecisionTree', 'MLP', 'GradientBoost', 'Ave']
    ...
    for i in range(0, len(learners)):
        ...
        AUC.append(round(auc_score, 3))
    AUC.append(round(np.array(AUC).mean(), 3))
    # AUC.append(round(np.array(AUC).std(), 3))
    results = pd.DataFrame({'AUROC': AUC})
    results.index = names
    print(results)
